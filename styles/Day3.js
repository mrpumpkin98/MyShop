import styled from "@emotion/styled";

export const Background = styled.div`
`;

export const Wrapper = styled.div`
  width: 1200px;
  /* height: 1847px; */
  border: 1px solid black;
  margin: 100px;
  padding-top: 80px;
  padding-bottom: 100px;
  padding-left: 102px;
  padding-right: 102px;
  display: flex;
  flex-direction: column;
  align-items: center;
  border: none;
  box-shadow: 0px 0px 10px gray;
`;

export const Container = styled.div`
width: 1000px;
`;

//##########################################################
//  ProfileBox 
//##########################################################
export const ProfileBox = styled.div`
border-bottom: 1px solid #BDBDBD;
display: flex;
justify-content: space-between;
align-items: center;
padding-bottom: 20px;

`;
export const ProfilePicture = styled.img`

`;

export const ProfileNameTie = styled.div`
margin-right: 730px;
`;

export const ProfileName = styled.div`
font-weight: 500;
font-size: 24px;
`;

export const PostDate = styled.div`
color: #828282;
`;

export const LinkTie = styled.div`
display: flex;
align-items: center;
`;

export const Link = styled.img`
cursor: pointer;
`;

export const Map = styled.img`
margin-left: 20px;
cursor: pointer;
`;

//##########################################################
//  HeaderBox 
//##########################################################

export const HeaderBox = styled.div`
margin-top: 80px;
`;

export const Title = styled.div`
font-weight: 700;
font-size: 36px;
`;

//##########################################################
//  AttachPicturesBox 
//##########################################################

export const AttachPicturesBox = styled.img`
width: 996px;
height: 480px;
margin-top: 40px;
`;

//##########################################################
//  ContentsBox 
//##########################################################

export const ContentsBox = styled.div`
margin-top: 30px;
margin-bottom: 100px;
`;

//##########################################################
//  YoutubeBox 
//##########################################################

export const YoutubeBox = styled.div`
display : flex;
justify-content: center;
margin-bottom: 100px;
`;

export const YoutubeVideo = styled.img`

`;

//##########################################################
//  LikeDislikeBox 
//##########################################################

export const LikeDislikeBox = styled.div`
display: flex;
justify-content: center;
`;

export const LikeDislikeTie = styled.div`
display: flex;
flex-direction: column;
justify-content: center;
align-items: center;
    margin: 0px 20px;
`;

export const LikeDislikeIcon = styled.img`
margin-bottom: 10px;
`;

export const LikeDislikeCount = styled.div`
color: #828282;
`;

//##########################################################
//  RegistrationBox 
//##########################################################

export const RegistrationBox = styled.div`
        margin: 100px;
        width: 1200px;
        display: flex;
        justify-content: center;
`;

export const ListButton = styled.button`
border: 1px solid #BDBDBD;
background-color: white;
font-weight: 500;
font-size: 16px;
width: 179px;
height: 45px;
margin: 0px 10px;
cursor: pointer;
`;

export const EditButton = styled.button`
border: 1px solid #BDBDBD;
background-color: white;
font-weight: 500;
font-size: 16px;
width: 179px;
height: 45px;
margin: 0px 10px;
cursor: pointer;
`;

export const DeleteButton = styled.button`
border: 1px solid #BDBDBD;
background-color: white;
font-weight: 500;
font-size: 16px;
width: 179px;
height: 45px;
margin: 0px 10px;
cursor: pointer;
`;

//##########################################################
//
//  Footer
//
//##########################################################

export const Footer = styled.div`
`;

export const FooterContainer = styled.div`
`;

export const CommentTitle = styled.div`
`;

//##########################################################
//  WriteCommentBox 
//##########################################################

export const WriteCommentBox = styled.div`
`;

export const InPutBox = styled.div`
`;

export const WriterInputPasswordInputTie = styled.div`
`;

export const WriterInput = styled.div`
`;

export const PasswordInput = styled.div`
`;

export const Scope = styled.div`
`;

export const TextTie = styled.div`
`;

export const CommentInput = styled.div`
`;

export const LetterRepair = styled.div`
`;

export const RegistrationButton = styled.div`
`;


//##########################################################
//  EditCommentBox 
//##########################################################

export const EditCommentBox = styled.div`
`;

export const RetouchButton = styled.div`
`;

//##########################################################
//  CommentBox
//##########################################################

export const CommentBox = styled.div`
`;